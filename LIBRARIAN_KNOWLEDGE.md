...
> **Updated:** 2026-06-13 — Rust router added, /reset endpoint, full server built
> **Repo:** `G:\llama.cpp` — HEAD `d65ad67` (before router work)
...
6. [Rust Router](#6-rust-router)
7. [Hard-Won Lessons](#7-hard-won-lessons)
8. [Troubleshooting Guide](#8-troubleshooting-guide)
9. [Known Issues](#9-known-issues)
10. [Detailed Git State](#10-detailed-git-state)
...
| File | Role | Notes |
|------|------|-------|
| `examples/server-mini/server-mini.cpp` | HTTP server source | `--alias` flag + `/reset` endpoint |
...
| File | Role | Notes |
|------|------|-------|
| `router/src/main.rs` | Rust router source | Session management, context packing |
| `router/Cargo.toml` | Rust dependencies | axum, tokio, reqwest, serde, etc. |
| `router/target/release/llama-router.exe` | Compiled binary | 4.8 MB, ~10 MB RAM |
...
| File | Role | Notes |
|------|------|-------|
| `build_vs/bin/Release/llama-server-mini.exe` | Compiled binary | 49.1 MB, rebuilt with `/reset` |
| `build_vs/bin/Release/llama-server.exe` | Full server | ~200 MB, VRAM-heavy |
| `router/target/release/llama-router.exe` | Router binary | 4.8 MB |
...
```powershell
# Full rebuild from scratch (llama.cpp)
cd G:\llama.cpp\build_vs
cmake .. -DGGML_VULKAN=ON -DLLAMA_CUDA=OFF -DLLAMA_BUILD_SERVER=ON
...
The RX 570 has 4 GB VRAM. One `llama-server-mini.exe` instance uses ~3.3 GB. Running two instances requires ~6.6 GB — impossible on this hardware. The full `llama-server.exe` is also too heavy (~3.4 GB+) and only achieves partial GPU offload.

**Solution:** A Rust router that maintains session state outside VRAM and feeds one bounded context to one llama.cpp worker.

### Architecture

```
Client (port 8080) → Rust Router → llama-server-mini (port 9120, localhost)
```

- Router: `router/target/release/llama-router.exe` (4.8 MB, ~10 MB RAM)
- Binds to `0.0.0.0:8080` (LAN-facing)
- llama.cpp binds to `127.0.0.1:9120` (localhost only)
- Router is the only network-exposed service

### What It Does

1. Listens on port 8080 with OpenAI-compatible API
2. Maintains per-session transcripts in RAM (keyed by `X-Librarian-Session` header)
3. Packs prompts: system rules + recent turns (default: 8) + current user message
4. Forwards to llama.cpp with mutex-serialized access
5. Calls `POST /reset` on llama.cpp when switching sessions
6. Returns `X-Librarian-Session` header in responses

### What It Does NOT Do

- No shell execution
- No file editing
- No task priority decisions
- No output promotion
- No code commits
...
See [`docs/WINDOWS-RUST-ROUTER.md`](docs/WINDOWS-RUST-ROUTER.md)
...
### Lesson 9: Two model instances don't fit in 4GB VRAM
...
### Lesson 10: Full llama.cpp server is too heavy for RX 570

The full server (`llama-server.exe`) uses more VRAM than the mini server due to unified KV cache, parallel slots, and additional buffers. On the RX 570 4GB, it can only achieve partial GPU offload (some layers on CPU), making it slower than the mini server. The mini server + router approach is the correct architecture.
...
| 7 | **No multi-model serving** | Single model per process | Must stop/start to switch models | Use router for session management |
| 8 | **Router sessions in-memory only** | No SQLite persistence yet | Session state lost on router restart | Restart router + llama.cpp |
| 9 | **No context summarization** | Router drops old turns | Long conversations lose early context | Add rolling summary (planned) |

---

## 10. Detailed Git State
...
### Priority: Router enhancements

The Rust router is the correct architecture for this hardware. Next steps:

1. **Add SQLite persistence** — Survive router restarts without losing session state
2. **Add context summarization** — Replace dropped turns with rolling summaries
...
### Medium priority: Embedding support
